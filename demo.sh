...
if [ "$SHOW_DASHBOARD_INFO" = true ]; then
    ...
    echo "If you are testing multiple underlying LLMs (like Qwen, DeepSeek, Claude, or GLM)"
    ...
    echo -e "  ${YELLOW}bun run answer -m \"opencode/deepseek-v4-pro\" -t CODING-water-sim${NC}"
    echo ""
    echo -e "  ${YELLOW}# 2. Evaluate model outputs and score completion${NC}"
    echo -e "  ${YELLOW}bun run evaluate -m \"opencode/deepseek-v4-pro\" -t CODING-water-sim${NC}"
    echo ""
