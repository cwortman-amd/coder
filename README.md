...
- **Power & Thermodynamic Profile (Prefill vs. Decode)**: Identifies identical instantaneous package power (**~296 W** sustained) for both phases at the card's TDP limit, but discovers a **94× Energy-per-Token Disparity** (**0.094 J/prefill token** vs. **8.87 J/decode token**). Documents that decode creates a persistent thermal soak on the 640 GB/s GDDR6 memory bus (**+13 °C hotter memory**, 87.6 °C vs 72.4 °C) compared to compute-dense prefill bursts.
