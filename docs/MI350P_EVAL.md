...
> **25 Sep 2026 — vLLM Quark MXFP4 is measured.** Canonical: [`MI350P_MXFP4_VLLM_EVAL.md`](MI350P_MXFP4_VLLM_EVAL.md). **Freeze HF control** (AITER unset, stock `M_LEQ_8`): **79.35 C1 / 553.58 C8**. Babel Read **3793 GB/s**; C1 is **40%** of that *conditional* roof — do not assign the rest to one kernel. Isolated ASM / ksplit=1 GEMM wins **regressed serving**. This 24 Sep document remains the **FP8 / competitive-lane** note.
...
**SGLang + MXFP4 is still unmeasured.** vLLM + Quark MXFP4 on this host **is** measured (25 Sep); see [`MI350P_MXFP4_VLLM_EVAL.md`](MI350P_MXFP4_VLLM_EVAL.md).
...
This section is **vLLM 0.30 ROCm × Qwen3.8-27B-FP8** (24 Sep), GPU 0, `max-model-len=9600`. The AMD ROCm 10 SGLang 0.5.15 image is local but has not served the model. Quark MXFP4 later served on **stock** `vllm/vllm-openai-rocm:latest` (no `local/vllm-mxfp4:gfx950` image required); numbers are in [`MI350P_MXFP4_VLLM_EVAL.md`](MI350P_MXFP4_VLLM_EVAL.md).
...
| MXFP4 (vLLM) | Quark AWQ MXFP4 + `AiterMxfp4LinearKernel` | **Complete 25 Sep:** 79.35 C1 / 553.58 C8; see `MI350P_MXFP4_VLLM_EVAL.md` |
...
4. Quark MXFP4 + native kernels. **vLLM path complete 25 Sep** (`MI350P_MXFP4_VLLM_EVAL.md`). SGLang MXFP4 still pending.  
